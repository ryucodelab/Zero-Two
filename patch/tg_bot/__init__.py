...
class ZeroTwoINIT:
    def __init__(self, parser: ConfigParser):
        ...
        # REDIS_URI/REDIS_URL takes priority when set (e.g. rediss://default:pass@host:port/0),
        # which is how most hosted Redis providers (Upstash, Redis Cloud, Railway, etc) give you
        # connection details. Falls back to the discrete HOST/PORT/DB/PASSWORD fields if no URI
        # is configured.
        self.REDIS_URI: str = self.parser.get("REDIS_URI", None) or self.parser.get(
            "REDIS_URL", None
        )
...
def get_redis_client(**overrides) -> Redis:
    """Build a Redis client honoring REDIS_URI/REDIS_URL when set, otherwise
    falling back to the discrete REDIS_HOST/PORT/DB/PASSWORD fields.

    Any module that needs its own Redis client (e.g. for decode_responses=True)
    should go through this instead of instantiating redis.Redis(...) directly,
    so a REDIS_URI configured by the user is actually respected everywhere.
    """
    if REDIS_URI:
        return Redis.from_url(REDIS_URI, **overrides)
    return Redis(
        host=REDIS_HOST,
        port=REDIS_PORT,
...
try:
    redis_conn.ping()
    if REDIS_URI:
        log.info("Redis connection successful (via REDIS_URI)")
    else:
        log.info(f"Redis connection successful ({REDIS_HOST}:{REDIS_PORT})")
